...
function formatMessageTime(dateString: Date, lastMessage = false) {
  ...
  if (lastMessage && currentDate.toDateString() !== date.toDateString()) {
    ...
    // If it's older than 3 hours, return AM/PM time
...
export function getMessageTime(message: ReduxMessage) {
  ...
  return formatMessageTime(dateCreated);
}
...
export function getLastMessageTime(messages: ReduxMessage[]) {
  ...
  return formatMessageTime(lastMessageDate, true);
}
